...
def to_upper_bound(req: str, python_version: str):
    try:
        ...
        if req_marker := requirement.marker:
            python_compatible = req_marker.evaluate(environment={"python_version": python_version})
        else:
            python_compatible = True
        if not python_compatible:
            return f"# INCOMPATIBLE_PYTHON(from-ci-image={python_version}): {req}"
...
def to_lower_bound(req: str, python_version: str):
    try:
        requirement = Requirement(req)
    except ValueError:
        return f"# UNPARSABLE: {req}"
...
def main():
    ...
    parser.add_argument(
        "--python-version",
        type=str,
        help="The python-version used by the CI container.",
        required=True,
    )
    ...
    modifier = partial(modifier, python_version=args.python_version)
