...
class DomainViewSet(
    ...
    def migrate(self, request, **kwargs):
        ...
        **IMPORTANT** This task will block all other tasks within the domain until the migration is
        ...
        the old backend until then. Note, when performing this operation on the default domain, Pulp
        will not use the values supplied to the endpoint, but rather the values found in the
        settings config.
        ...
        if instance.name == "default":
            data = {
                ...
                "storage_settings": DomainSerializer.get_default_domain_settings(settings),
            }
