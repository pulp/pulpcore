...
class RepositoryVersionContentFilter(Filter):
    ...
    def filter(self, qs, value):
        ...
        version_tuples = itertools.zip_longest(
            version_added, version_removed, fillvalue=repository.last_version + 1
        )
...
class RemoteFilter(BaseFilterSet):
    ...
    class Meta:
        model = Remote
        fields = {"name": NAME_FILTER_OPTIONS, "_last_updated": DATETIME_FILTER_OPTIONS}
...
class ExporterFilter(BaseFilterSet):
    ...
    class Meta:
        model = Exporter
        fields = {"name": NAME_FILTER_OPTIONS, "last_export": DATETIME_FILTER_OPTIONS}
