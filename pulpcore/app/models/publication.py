...
class EnvVarHeaderContentGuard(ContentGuard, AutoAddObjPermsMixin):
    ...
    def permit(self, request):
        ...
        header_content = request.headers.get(self.header_name)
        if not header_content:
            message = _("Access not allowed. Header {} not found.").format(self.header_name)
        ...
        try:
            ...
            message = _("Access not allowed - Header content is not Base64 encoded.")
            _logger.debug(message)
        ...
        try:
            ...
            message = _("Access not allowed - Header content is not valid UTF-8.")
            _logger.debug(message)
            raise PermissionError(message) from None
        ...
        if not hmac.compare_digest(
            ...
            _logger.warning("Access not allowed. Header value does not match environment variable.")
            raise PermissionError(_("Access denied."))
