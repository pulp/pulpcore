...
def analyze_sig_subpackets(attr, data):
    for packet in subpacket_iter(data):
        packet_type = packet["type"]
        body = packet["body"]
        if packet_type == 2:
            attr["created"] = timezone.datetime.fromtimestamp(
                int.from_bytes(body, "big")
            ).astimezone()
        elif packet_type == 3:
            attr["expiration_time"] = timezone.timedelta(seconds=int.from_bytes(body, "big"))
        elif packet_type == 9:
            attr["key_expiration_time"] = timezone.timedelta(seconds=int.from_bytes(body, "big"))
        elif packet_type == 16:
            attr["issuer"] = body.hex()
        elif packet_type == 28:
            attr["signers_user_id"] = body.decode()
...
def analyze_signature(data, pubkey, signed_packet_type, signed_packet):
    ...
    if version == 3:
        ...
        hash_algorithm = HASH_ALGORITHMS.get(data[3], "unknown")
        ...
        attr = {"sha256": sha256, "signature_type": signature_type, "raw_data": data}
        # Hashed Subpackets
        analyze_sig_subpackets(attr, hashed_data)
        # Unhashed Subpackets
        analyze_sig_subpackets(attr, unhashed_data)
        return attr
    else:
        ...
        if pubkey_algorithm and "format" in pubkey_algorithm:
            ...
            for item_type in pubkey_algorithm["format"]:
                if item_type == "m":
                    ...
                    # OID
                    oid = extract_oid(key_data[pos:])
                    pos += len(oid["raw"])
                elif item_type == "k":
                    # KDF
                    # TODO more analysis
                    kdf = extract_oid(key_data[pos:])
                    pos += len(kdf["raw"])
                else:
                    raise ValueError("")
            if version == 4:
