...
class TestFlushEvent:
    """The flush event signals downstream stages to flush."""

    @pytest.mark.asyncio
    async def test_flush_event_set_when_blocked(self):
        budget = ArtifactResourceBudget(max_items=1)
        assert not budget.flush_event.is_set()
        await budget.acquire(0)
        ...
        await asyncio.sleep(0.05)
        assert budget.flush_event.is_set(), "flush_event should be set when acquire blocks"

        budget.release(0)
        await asyncio.sleep(0.05)
        assert not budget.flush_event.is_set(), "flush_event should clear after release"
        task.cancel()

    @pytest.mark.asyncio
    async def test_flush_event_not_set_when_budget_available(self):
