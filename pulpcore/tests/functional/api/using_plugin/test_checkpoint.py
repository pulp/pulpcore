...
def content_factory(tmp_path_factory, pulpcore_bindings, file_bindings, monitor_task):
    def _content_factory(name):
        file = tmp_path_factory.mktemp("content") / name
        file.write_text(str(uuid.uuid4()))
        artifact = pulpcore_bindings.ArtifactsApi.create(str(file))
        artifact_attrs = {"artifact": artifact.pulp_href, "relative_path": name}
        return monitor_task(
