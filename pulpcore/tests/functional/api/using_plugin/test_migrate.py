...
def test_migrate_bad_settings(domains_api_client, pulp_domain_enabled):
...
def test_migrate_default_domain(
    domains_api_client, backend_settings_factory, monitor_task, pulp_domain_enabled
):
    ...
    if pulp_domain_enabled:
        kwargs["pulp_domain"] = domain.name
    task = monitor_task(domains_api_client.migrate(body, **kwargs).task)
