...
def test_filter_tasks_using_worker__in_filter(tasks_api_client, dispatch_task, monitor_task):
    task1_href = dispatch_task("pulpcore.app.tasks.test.sleep", (0,))
    task2_href = dispatch_task("pulpcore.app.tasks.test.sleep", (0,))
...
def test_task_created_by(dispatch_task, monitor_task, gen_user, anonymous_user):
    ...
    task = monitor_task(dispatch_task("pulpcore.app.tasks.test.sleep", (0,)))
    assert task.created_by.endswith("/1/")

    # Test w/ new user, user_id != 1
    user = gen_user()
    with user:
        task_href = dispatch_task("pulpcore.app.tasks.test.sleep", (0,))
