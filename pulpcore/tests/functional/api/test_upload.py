...
def test_upload_synchronous(file_content_api, gen_user, file_fixtures_root):
    ...
    with gen_user(model_roles=["file.file_uploader"]):
        ...
        package = file_content_api.upload(**upload_attrs)
        ...
        duplicate_package = file_content_api.upload(**upload_attrs)
    ...
    with gen_user(model_roles=[]), pytest.raises(FileApiException) as ctx:
        labels = {"key_1": "value_1"}
        upload_attrs = {"file": str(file_path), "relative_path": "1.iso", "pulp_labels": labels}
        file_content_api.upload(**upload_attrs)
