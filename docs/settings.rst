...
^^^^^^^^^^^^^^^^^^^
    ...
    Pulp has a new experimental worker type which is startable with the ``pulp-worker`` command. If
    ``True`` Pulp is expecting to have its workers started with ``pulp-worker``. Defaults to
    ``False``.
